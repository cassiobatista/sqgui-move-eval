...
if __name__ == '__main__':
    ...
    plt.imshow(np.array(cf2, dtype=np.int16),cmap=plt.cm.Oranges)
    plt.xticks(np.arange(0,4), ['Down', 'Up', 'Left', 'Right'], fontsize=24)
    plt.yticks(np.arange(0,3), ['Down', 'Left', 'Right'], va='center', fontsize=24, rotation=90)
    denormalize(cf)
    for i in range(4):
        for j in range(3):
            if True: #cf[j][i] > 0:
